...
//   $('Read SkewConfig').all()    → rows: {partnerType, displayName, promptInstruction, active}
//   $('Read Subscribers').all()   → rows including {partnerType, active}
//
// Output: one item per variant → {partnerType, displayName, promptInstruction}

const skewRows = $('Read SkewConfig').all().map(i => i.json);
const subRows = $('Read Subscribers').all().map(i => i.json);
